...
local PJSummaryController = class("PJSummaryController", Controller):include(HasSignals)

function PJSummaryController:initialize(data)
...
function PJSummaryController:viewDidLoad()
...
function PJSummaryController:clickBack()
    if self.data.autoShare then
        self.emitter:emit('back')
    else
...
function PJSummaryController:clickShare(t)
