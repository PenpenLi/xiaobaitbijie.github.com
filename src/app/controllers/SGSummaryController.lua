...
local SGSummaryController = class("SGSummaryController", Controller):include(HasSignals)

function SGSummaryController:initialize(data)
...
function SGSummaryController:viewDidLoad()
...
function SGSummaryController:clickBack()
    if self.data.autoShare then
        self.emitter:emit('back')
    else
...
function SGSummaryController:clickShare(t)
...
function SGSummaryController:clickShare_to_Xianliao()
    local CaptureScreen = require('app.helpers.capturescreen')
    local SocialShare = require('app.helpers.SocialShare')
