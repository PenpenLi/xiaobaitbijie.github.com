...
function DKChatView:initialize()
end
...
function DKChatView.getChatsTbl()
    return chatsTbl
end

function DKChatView:getEmojiTbl()
    return emojiTbl
end

function DKChatView:layout(desk)
    self.desk = desk
...
function DKChatView:initShortcutList()
...
function DKChatView:freshBtnState(mode)
...
function DKChatView:freshListState(mode)
    local chattingRecord = self.MainPanel:getChildByName('chattingRecord')
    local mode = mode or 'shortcut'
    if mode == 'shortcut' then
...
function DKChatView:freshRecordList(msg)
...
function DKChatView:freshHeadImg(item, headUrl)
...
function DKChatView:getChatEditBoxInfo() 
    local text = self.chatEditBox:getText()
    return text 	
end

function DKChatView:freshChatEditBox(content, enable)
    enable = enable or false
    self.chatEditBox:setText(content)
    self.chatEditBox:setEnabled(enable)
end

function DKChatView:initEmojiList()
...
function DKChatView:setBtnClickEvent(item, line, col)
    for i = 1, col do
...
function DKChatView:biaoqingRemind(idx, bool)
