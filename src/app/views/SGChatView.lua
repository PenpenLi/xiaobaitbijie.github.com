...
function SGChatView:initialize()
end
...
function SGChatView.getChatsTbl()
    return chatsTbl
end

function SGChatView:getEmojiTbl()
    return emojiTbl
end

function SGChatView:layout(desk)
    self.desk = desk
...
function SGChatView:initShortcutList()
...
function SGChatView:freshBtnState(mode)
...
function SGChatView:freshListState(mode)
    local chattingRecord = self.MainPanel:getChildByName('chattingRecord')
    local mode = mode or 'shortcut'
    if mode == 'shortcut' then
...
function SGChatView:freshRecordList(msg)
...
function SGChatView:freshHeadImg(item, headUrl)
...
function SGChatView:getChatEditBoxInfo() 
    local text = self.chatEditBox:getText()
    return text 	
end

function SGChatView:freshChatEditBox(content, enable)
    enable = enable or false
    self.chatEditBox:setText(content)
    self.chatEditBox:setEnabled(enable)
end

function SGChatView:initEmojiList()
...
function SGChatView:setBtnClickEvent(item, line, col)
    for i = 1, col do
...
function SGChatView:biaoqingRemind(idx, bool)
