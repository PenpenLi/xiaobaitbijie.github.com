...
function PJSummaryView:layout(data)
...
function PJSummaryView:getWinner(tbl)
    local score, key
...
function PJSummaryView:getloser(tbl) -- 获取土豪（输最多的人）
    local score, key
    for k, v in pairs(tbl) do
        if score == nil and key == nil then
            score, key = v.score, k
...
function PJSummaryView:loadSummaryList(data)
...
function PJSummaryView:freshItem(item, player, record, win, lose, playernum)
    if record then
        -- 抢庄, 庄家, 推注次数
...
function PJSummaryView:startCsdAnimation( node, csbName, isRepeat, timeSpeed)
...
function PJSummaryView:startPlistAnimation( plistName, sprite, length)
