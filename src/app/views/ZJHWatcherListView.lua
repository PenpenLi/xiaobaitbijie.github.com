...
local ZJHWatcherListView = {}
function ZJHWatcherListView:initialize()

end

function ZJHWatcherListView:layout()
...
function ZJHWatcherListView:freshListView(data)
...
function ZJHWatcherListView:freshCnt(cnt)
    self.cnt:setString(string.format("(%s)", cnt))
    self.cnt:setVisible(cnt > 0)
end


function ZJHWatcherListView:freshListItem(row, column, data)
    if column == 1 then
...
-- ============= item view =============
function ZJHWatcherListView:freshHeadImg(item, headUrl)
...
function ZJHWatcherListView:freshNameAndId(item, name, id)
