...
function PJChatView:initialize()
end
...
function PJChatView.getChatsTbl()
    return chatsTbl
end

function PJChatView:getEmojiTbl()
    return emojiTbl
end

function PJChatView:layout(desk)
    self.desk = desk
...
function PJChatView:initShortcutList()
...
function PJChatView:freshBtnState(mode)
...
function PJChatView:freshListState(mode)
    local chattingRecord = self.MainPanel:getChildByName('chattingRecord')
    local mode = mode or 'shortcut'
    if mode == 'shortcut' then
...
function PJChatView:freshRecordList(msg)
...
function PJChatView:freshHeadImg(item, headUrl)
...
function PJChatView:getChatEditBoxInfo() 
    local text = self.chatEditBox:getText()
    return text 	
end

function PJChatView:freshChatEditBox(content, enable)
    enable = enable or false
    self.chatEditBox:setText(content)
    self.chatEditBox:setEnabled(enable)
end

function PJChatView:initEmojiList()
...
function PJChatView:setBtnClickEvent(item, line, col)
    for i = 1, col do
...
function PJChatView:biaoqingRemind(idx, bool)
