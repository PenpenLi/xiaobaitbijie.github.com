...
function SGSummaryView:layout(data)
...
function SGSummaryView:getWinner(tbl)
    local score, key
...
function SGSummaryView:getloser(tbl) -- 获取土豪（输最多的人）
    local score, key
    for k, v in pairs(tbl) do
        if score == nil and key == nil then
            score, key = v.score, k
...
function SGSummaryView:loadSummaryList(data)
...
function SGSummaryView:freshItem(item, player, record, win, lose, playernum)
    if record then
        -- 抢庄, 庄家, 推注次数
...
function SGSummaryView:startCsdAnimation( node, csbName, isRepeat, timeSpeed)
...
function SGSummaryView:startPlistAnimation( plistName, sprite, length)
