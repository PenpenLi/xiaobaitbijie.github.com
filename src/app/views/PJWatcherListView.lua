...
local PJWatcherListView = {}
function PJWatcherListView:initialize()
	
end

function PJWatcherListView:layout()
...
function PJWatcherListView:freshListView(data)
...
function PJWatcherListView:freshCnt(cnt)
	self.cnt:setString(string.format( "(%s)",cnt))
	self.cnt:setVisible(cnt>0)
end


function PJWatcherListView:freshListItem(row, column, data)
	if column == 1 then
...
-- ============= item view =============
function PJWatcherListView:freshHeadImg(item, headUrl)
...
function PJWatcherListView:freshNameAndId(item, name, id)
