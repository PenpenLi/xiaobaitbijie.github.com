...
function DKSummaryView:layout(data)
...
function DKSummaryView:getWinner(tbl)
    local score, key
...
function DKSummaryView:getloser(tbl) -- 获取土豪（输最多的人）
    local score, key
    for k, v in pairs(tbl) do
        if score == nil and key == nil then
            score, key = v.score, k
...
function DKSummaryView:loadSummaryList(data)
...
function DKSummaryView:freshItem(item, player, record, win, lose, playernum)
    if record then
        -- 抢庄, 庄家, 推注次数
...
function DKSummaryView:startCsdAnimation( node, csbName, isRepeat, timeSpeed)
...
function DKSummaryView:startPlistAnimation( plistName, sprite, length)
