...
local ZJHSummaryView = {}

function ZJHSummaryView:layout(data)
    ...
    local panel1 = mainPanel:getChildByName('Panel1')
    ...
    local item1 = mainPanel:getChildByName('item1')
    local item2 = mainPanel:getChildByName('item2')
    local item3 = mainPanel:getChildByName('item3')
    self.item1 = item1
    self.item2 = item2
    self.item3 = item3
...
function ZJHSummaryView:getWinner(tbl)
    local score, key
...
function ZJHSummaryView:getloser(tbl) -- 获取土豪（输最多的人）
    local score, key
    for k, v in pairs(tbl) do
        if score == nil and key == nil then
            score, key = v.score, k
...
function ZJHSummaryView:loadSummaryList(data)
...
function ZJHSummaryView:freshItem(item, player, record, win, lose, playernum)
    if record then
        -- 抢庄, 庄家, 推注次数
...
function ZJHSummaryView:startCsdAnimation( node, csbName, isRepeat, timeSpeed)
...
function ZJHSummaryView:startPlistAnimation( plistName, sprite, length)
