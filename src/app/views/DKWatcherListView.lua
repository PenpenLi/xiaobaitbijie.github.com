...
local DKWatcherListView = {}
function DKWatcherListView:initialize()
	
end

function DKWatcherListView:layout()
...
function DKWatcherListView:freshListView(data)
...
function DKWatcherListView:freshCnt(cnt)
	self.cnt:setString(string.format( "(%s)",cnt))
	self.cnt:setVisible(cnt>0)
end


function DKWatcherListView:freshListItem(row, column, data)
	if column == 1 then
...
-- ============= item view =============
function DKWatcherListView:freshHeadImg(item, headUrl)
...
function DKWatcherListView:freshNameAndId(item, name, id)
