...
local SGWatcherListView = {}
function SGWatcherListView:initialize()
	
end

function SGWatcherListView:layout()
...
function SGWatcherListView:freshListView(data)
...
function SGWatcherListView:freshCnt(cnt)
	self.cnt:setString(string.format( "(%s)",cnt))
	self.cnt:setVisible(cnt>0)
end


function SGWatcherListView:freshListItem(row, column, data)
	if column == 1 then
...
-- ============= item view =============
function SGWatcherListView:freshHeadImg(item, headUrl)
...
function SGWatcherListView:freshNameAndId(item, name, id)
