...
local SGDesk = class("Desk"):include(HasSignals)
local ShowWaiting = require('app.helpers.ShowWaiting')
local cardHelper = require('app.helpers.card')
local tools = require('app.helpers.tools')
local EventCenter = require("EventCenter")
local Agent = require('app.libs.sangong.SGAgent')
local Gameplay = require('app.libs.sangong.SGGameplay')
local GameLogic = require('app.libs.sangong.SGGameLogic')
local app = require("app.App"):instance()


function SGDesk:initialize()
    HasSignals.initialize(self)

    -- 桌子
    self.gameIdx = 34
    self.DeskName = 'sangong'
...
function SGDesk:disposeListens()
...
function SGDesk:bindMsgHandles()
    local app = require("app.App"):instance()
    self:disposeListens()
    ...
    self.listens = {
        ...
        --PutMoney
        app.conn:on(self.DeskName .. '.StatePutLimit', function(msg)
            self.gameplay:setState('PutLimit', msg.tick)
            self.emitter:emit('StatePutLimit', msg)
        end),

        app.conn:on(self.DeskName .. '.putLimit', function(msg)
            --msg.putInfo
            local info = self:getPlayerInfo(msg.uid)
            if not info then return end
            info.player:setThisPutOpt(msg.putInfo)
            ...
            self.emitter:emit('putLimit', cMsg)
        end),

        app.conn:on(self.DeskName .. '.somebodyPutLimit', function(msg)
            -- 押注
            local info = self:getPlayerInfo(msg.uid)
            if not info then return end
            info.player:setPutscore(msg.score)
            ...
            info.player:setCanPutMoney(false)
            local cMsg = {
                msgID = 'somebodyPutLimit',
                info = info,
                score = msg.score,
                putFlag = msg.pushFlag,
                tuizhuflag = msg.tuizhuflag
            }
            self.emitter:emit('somebodyPutLimit', cMsg)
        end),
...
function SGDesk:onCustomSwitch() 
    app:switch('SGDeskController', self.DeskName)
end

function SGDesk:onPutMoney(msg)
    self.emitter:emit('freshBettingBar', msg)
    self.emitter:emit('bettingTimerStart')
end

function SGDesk:onQiangZhuang()
...
function SGDesk:initViewKey() -- virtual 
...
function SGDesk:getViewKey(pos) -- virtual 
    return self.tabViewKey[pos]
end

function SGDesk:getViewKeyData() 
    return self.tabViewKey
end

-- ============ onnetmsg ============

function SGDesk:onSynDeskData(msg)
    self:resetDesk()
...
function SGDesk:onSomebodyTrusteeship(msg)
...
function SGDesk:onDeskRecord(msg)
    self.tabDeskRecord = msg.data or {}
end

function SGDesk:onWatcherList(msg)
    self.tabWatcher = msg.data or {}
end

function SGDesk:onChatInGame(msg)  
...
function SGDesk:onChatList(msg)
    self.tabChatList = msg or {}
end

function SGDesk:onSummary(msg)
    -- 单局结算
    for k,v in pairs(msg.data) do
        local info = self:getPlayerInfo(k)
        if info then
            info.player:setSummaryCardData(v.dealList)
...
function SGDesk:onAutoOperation(msg)
    local info = self:getPlayerInfo(msg.uid)
    if info then
...
function SGDesk:onOvergame(msg)
...
function SGDesk:putLimit(value)
    local app = require("app.App"):instance()
    local conn = app.conn
	local msg = {
		msgID = self.DeskName .. '.putLimits',
		score = value
	}
	app.conn:send(msg)
end

function SGDesk:watcherList()
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
      msgID = 'watcherList',
...
function SGDesk:sitDown(deskId, buyHorse) -- luacheck:ignore
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
function SGDesk:getLastVoice(msg) -- luacheck:ignore
    local app = require("app.App"):instance()
    local conn = app.conn
    local tmsg = {
...
function SGDesk:quit() -- luacheck:ignore
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
function SGDesk:answer(answer)--luacheck:ignore
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
function SGDesk:cancelTrusteeship()
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
function SGDesk:requestTrusteeship()
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
--智能托管
function SGDesk:sendTrusteeshipMsg(bool, msg)
    local app = require("app.App"):instance()
    local conn = app.conn
    local rmsg = {
...
--请求获取托管状态
function SGDesk:requestTrusteeshipMsg()
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
function SGDesk:requestSitdown()
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
function SGDesk:autoOperation(msg)
    local app = require("app.App"):instance()
    local conn = app.conn
    local tmsg = {
...
function SGDesk:uploadVoiceSuccess(filename, total) --luacheck
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
        msgID = 'playVoice',
...
function SGDesk:leaveRoom() --luacheck
...
function SGDesk:dismiss() --luacheck
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
function SGDesk:reloadData() --luacheck
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
function SGDesk:qiangzhuang(qiangNum) --luacheck
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
function SGDesk:showCard() --luacheck
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
--切到后台
function SGDesk:enterBackground() --luacheck
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
--设置作弊信息
function SGDesk:setCheatData(mode)
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
--设置定位信息
function SGDesk:setLocationInfo(info)
    local app = require("app.App"):instance()
    local conn = app.conn
    local msg = {
...
--@baseInfo
function SGDesk:getMaxPlayerCnt() --virtual
    local deskInfo = self.tabBaseInfo.deskInfo 
    return deskInfo.maxPeople
end

function SGDesk:getDeskInfo()
    -- 游戏规则
    return self.tabBaseInfo.deskInfo 
end

function SGDesk:getgroupInfo()
    -- 俱乐部信息
    return self.tabBaseInfo.groupInfo 
end

function SGDesk:getGameplayIdx()
    return self.tabBaseInfo.deskInfo.gameplay 
end

function SGDesk:getPeopleSelectIdx()
    return self.tabBaseInfo.deskInfo.peopleSelect 
end

function SGDesk:getWanglai()
    return self.tabBaseInfo.deskInfo.advanced[5] or 0 
end

function SGDesk:getDeskMode()
    return self.tabBaseInfo.deskInfo.deskMode 
end

function SGDesk:getDismissInfo()
    -- 解散信息
    return self.overSuggest, self.overTickInfo
end

function SGDesk:setDismissInfo(overSuggest, overTickInfo)
    self.overSuggest = overSuggest
    self.overTickInfo = overTickInfo
end

function SGDesk:getOwnerInfo()
    -- 房主信息
    return {
...
function SGDesk:getReadyPlayerCnt()
    local cnt = 0
...
--@player
function SGDesk:getPlayerInfo(uid, viewKey) --virtual
    local player
    if not self.tabPlayer then return end
...
function SGDesk:isMeRobot()
    local app = require("app.App"):instance()
    return app.session.user:isMeRobot()
end

function SGDesk:getBottomPlayer()--弃用接口
    -- 主视角玩家
...
function SGDesk:getTick()
    return self.gameplay:getTick()
end

--@deskRecord
function SGDesk:getDeskRecord(msg)
    return self.tabDeskRecord
end

--@watchList
function SGDesk:getWatcherList()
    return self.tabWatcher
end

--@chatList
function SGDesk:getChatList()
...
--////////////////////////////////////////////////////////////////////////////////////////////////
-- !private
function SGDesk:initPlayer(uid, agentPack, lookupFlag, lookupCID, initSelf) 
    -- 初始化tabPlayer
...
function SGDesk:getLastFreeChairID()
    --最后的空位
    local tabValidCID = {}
    local validCnt = 0
